...
4. **Konta personelu** (brak UI tożsamości — zakładane ręcznie):
   ...
   - Powiąż konto z tenantem (snippet na końcu `seed.sql`):
     ```sql
     update auth.users
...
5. **Realtime / Broadcast** działa domyślnie — kanały są publiczne, bez dodatkowej konfiguracji.
6. Z **Project Settings → API** skopiuj `Project URL`, `anon public key`, `service_role key` → do `.env.local`.
...
Szybki test logiki bookingu end-to-end (bez UI):
```bash
...
Cookie (`locale`) trzyma język; Server Components czytają je i przekazują wycinek słownika w dół (`lib/i18n.ts`). `LanguageSwitcher` przełącza cookie i robi `router.refresh()`. Komunikaty błędów z Server Actions też są lokalizowane. Bez zewnętrznej zależności i18n.
...
lib/
  ...
  i18n.ts                       # słowniki PL/EN + helpery
components/
  ui/                           # button/input/label/card/table (brutalist)
  LanguageSwitcher.tsx          # przełącznik PL/EN (cookie + refresh)
supabase/
